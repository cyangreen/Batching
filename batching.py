...
resource_selected = st.multiselect('Select resources', resource)
mask_resource = data['resource'].isin(resource_selected)
data = data[mask_resource]

# Select activities
activity = data['activity'].unique()
activity_selected = st.multiselect('Select activities', activity)
...
st.text("Once features have been engineered, the event log looks like this")
#Show the new dataframe
st.dataframe(data)

#Change the time between activites that should form a batch
m=st.slider("Time in minutes between activites that should form a batch",1,30)
...
scatter_chart = st.altair_chart(
    alt.Chart(df_scatterplot)
        .mark_circle(size=60)
        .encode(x='Concurrent batching', y='Lead time', color='Service Time')
        .interactive()
)
